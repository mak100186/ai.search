...
public class Maze
{
    ...
    public Maze(string filename)
    {
        ...
        if (string.IsNullOrWhiteSpace(assemblyDirectory)) throw new Exception("Could not locate assembly directory");

        _filename = filename;
        var fullFilePath = Path.Combine(assemblyDirectory, "Mazes/Resources", filename);
        ...
        for (var y = 0; y < _height; y++)
        {
            ...
            if (line.Length > _width) _width = line.Length;
        ...
        if (startPoints != 1) throw new Exception("maze must have exactly one start point");

        if (endPoints != 1) throw new Exception("maze must have exactly one end point");
    }
    ...
    public void SaveImage()
    {
        ...
        var filePath = Path.Combine(assemblyDirectory, $"{_algorithmType}-{_filename}.png");
    ...
    private List<Action> Neighbors(Point state)
    {
        ...
        {
            new(ActionTypes.Up, new Point(col, row - 1)),
            new(ActionTypes.Down, new Point(col, row + 1)),
            new(ActionTypes.Left, new Point(col - 1, row)),
            new(ActionTypes.Right, new Point(col + 1, row))
        };
    ...
    public int Solve(AlgorithmTypes algorithm)
    {
        ...
        while (true)
        {
            if (frontier.Empty()) throw new Exception("no solution found");
